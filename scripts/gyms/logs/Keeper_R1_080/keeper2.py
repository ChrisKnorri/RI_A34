...
class GoalkeeperEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.goalkeeper_status in [1, 2, 3]:
            print(f"action changet to get up {action}")
            action = 4
        ...
        for i in range(loop):
            ...
            if action not in [0, 5]:
                ...
                if self.ready:
                    if action in [1, 2, 3, 4] and self.ready and self.step_counter < BALL_STARTS_ITER:
                        reward += -0.5

                    if action == 4:
                        print(f"get up finished in : {self.step_counter - start}")
                ...
                intersection_point = np.clip(intersection_point[0], -15, -13.5), np.clip(intersection_point[1], -0.9, 0.9)
