...
class GoalkeeperEnv(gym.Env):
    def __init__(self, ip, server_p, monitor_p, r_type, enable_draw) -> None:
        ...
        self.stuck_limit = 5  # Maximum allowed iterations with no displacement before reset
        ...
        self.observation_space = spaces.Box(
            ...
            high=np.array([50, 30, 30, 30, -30, 30, 10, 10, len(action_dict), 400,1]),  # Max values
            dtype=np.float32
        )
    ...
    def spawn_ball(self):
        ...
        ball_velocity = (
            ...
            random.uniform(2.6, 3.5)  # Slight elevation
        )
    ...
    def observe(self):
        ...
        ball_vel_x, ball_vel_y, ball_vel_z = world.get_ball_abs_vel(1)[:3]
        ...
        self.obs = [ball_pos_x, ball_pos_y, ball_pos_z, ball_vel_x, ball_vel_y, ball_vel_z,
                    keeper_pos_x, keeper_pos_y, self.goalkeeper_status, self.ball_moved]
    ...
    def step(self, action):
        ...
        if action not in [0, 5]:
            ...
            if action in [1, 2, 3, 4] and self.step_counter < 2:
                reward += -1

            if action in [1, 2]:
                if ball_distance < proximity_threshold:
                    reward += -0.1
                if self.goalkeeper_status == 5: #shift then dive
                    reward += 0.1
                else:
                    # Reward for correct dive direction
                    if dive_direction is not None and behavior_name == dive_direction:
        ...
        # Check if episode is done
                ...
                (self.step_counter > 0 and self.step_counter >= 20) or
                (self.step_counter > 2 and (self.is_goal(b) or self.is_save(bh) or self.is_miss(b)))
        )
