...
class Fall(gym.Env):
    def __init__(self, ip, server_p, monitor_p, r_type, enable_draw) -> None:
        ...
        self.observation_space = gym.spaces.Box(low=np.full(len(self.obs), -np.inf, np.float32),
                                                high=np.full(len(self.obs), np.inf, np.float32), dtype=np.float32)

        # Action space
        MAX = np.finfo(np.float32).max
        no_of_actions = self.no_of_joints
        self.action_space = gym.spaces.Box(low=np.full(no_of_actions, -MAX, np.float32),
                                           high=np.full(no_of_actions, MAX, np.float32), dtype=np.float32)
    ...
    def observe(self):
        #todo find out how to observe what is needed
